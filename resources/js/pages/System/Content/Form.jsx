...
function Form({ DataEdit, type, setToggle, cancle, close }) {
  ...
  const HandleSubmit = (e) => {
    e.preventDefault()
    var url = '/api/content/store'
    if (type == 'update') {
      url = `/api/content/${edit?.uuid}/update`
    }
    const headers = {
  ...
  return (
    <div className='bg-white rounded-lg p-6 mb-2'>
      ...
      <div className="flex justify-between items-center">
        <span className='font-semibold text-xl'>Content {type == 'create' ? 'Create' : "Edit"}</span>
      ...
      <form onSubmit={HandleSubmit} >
        <div className="flex gap-x-2">
          <div className="flex flex-col w-full">
            ...
            <input id='title' type="text" className='border-[1px] border-solid rounded-md border-black focus:outline-blue-500 px-2 py-1' name='title' value={DataForm.title} onChange={HandleChange} autoComplete='off' />
          </div>
          <div className="flex flex-col w-full">
            <label htmlFor="title" className='font-semibold'>Body : <span className='text-red-600 font-semibold'>*</span></label>
            <textarea id='title' type="text" className='border-[1px] border-solid rounded-md border-black focus:outline-blue-500 px-2 py-1' name='body' value={DataForm.body} onChange={HandleChange} autoComplete='off' />
          </div>
        </div>
