...
function Index() {
  ...
  const GetContent = (uuid) => {
    setToggle(false)
    var url = `/api/content/${uuid}/edit`
  ...
  const HandleDelete = (uuid) => {
    var url = `/api/content/${uuid}/delete`
    Swal.fire({
      title: 'Are you sure?',
      ...
      if (result.isConfirmed) {
        ...
        axios.delete(url).then(res => {
          if (res.data.success === true) {
            ...
            setTimeout(() => {
              SwitchPage('/api/content?page=', page)
            }, 300);
          }
        }).catch(error => {
          Swal.fire({
    ...
    <>
      ...
      <div className="bg-white rounded-lg p-6 shadow-lg">
        <div className="flex justify-between items-center">
          <span className='font-semibold text-xl'>Content Data</span>
        ...
        <div className="overflow-y-auto">
              ...
              {block ? (<Loading colSpan={6} />) :
                (Content?.data?.map((item, index) => (
                  <tr className={`${index % 2 ? 'bg-gray-100' : 'bg-white'} h-14 hover:bg-gray-200`} key={item.id}>
                    <td className='text-center px-2 font-semibold'>{index + Content.from}</td>
                    <td>
                    ...
                    <td>
                      <div className="flex justify-center gap-x-1">
                        <Edit onClick={() => { GetContent(item.uuid) }} />
                        <Delete onClick={() => { HandleDelete(item.uuid) }} />
                      </div>
                    </td>
                  </tr>)
                ))}
              {Content?.data?.length == 0 && !block ? (
          ...
          <div className="flex justify-between mt-2">
            <span className='font-semibold'>Page {Content.current_page ?? null} of {Content?.last_page ?? null}</span>
            <div className="flex">
              {page != 1 ? (
